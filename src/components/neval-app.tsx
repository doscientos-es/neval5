...
import { createProduct, createProductFamily, createTaxRate } from "@/app/actions/catalog";
import { changeOrderStatus, convertQuote, createManualOrder, createQuote, duplicateOrder } from "@/app/actions/commercial";
  ...
  return <main className="min-h-screen bg-surface text-white lg:flex">
    ...
    <section className="min-w-0 flex-1 lg:ms-72">
      ...
      <div className="mx-auto max-w-[1550px] p-5 lg:p-8">
        ...
        {section === "Dashboard" ? <Dashboard data={dashboard} setSection={setSection} /> : section === "Clientes" ? <ClientsView clients={visibleClients} query={query} setQuery={setQuery} onAdd={() => setShowNewClient(true)} onEdit={setEditingClient} onArchive={removeClient} /> : section === "Configuración" ? <CatalogView catalog={catalog} onAdd={() => setShowNewProduct(true)} onFamily={() => setShowNewFamily(true)} onTax={() => setShowNewTax(true)} /> : section === "Presupuestos" ? <QuotesView quotes={commercial?.quotes ?? []} onConvert={(id) => runCommercial(() => convertQuote(id))} onAdd={() => setShowNewQuote(true)} /> : section === "Pedidos" ? <OrdersView orders={commercial?.orders ?? []} onStatus={(id, status) => runCommercial(() => changeOrderStatus(id, status))} onDuplicate={(id) => runCommercial(() => duplicateOrder(id))} /> : section === "Compras" || section === "Almacén" ? <InventoryView section={section} inventory={inventory} onSupplier={() => setShowNewSupplier(true)} onReceive={setReceipt} /> : <ModuleView section={section} onAction={() => setNotice("Módulo en preparación.")} />}
      </div>
    </section>
