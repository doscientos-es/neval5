...
export async function POST(request: Request) {
  ...
  const safeCustomerId = typeof customerId === "string" ? customerId : null; const safeOrderId = typeof orderId === "string" ? orderId : null;
  ...
  const { data, error } = await supabase.from("attachments").insert({ organization_id: membership.organization_id, customer_id: safeCustomerId, order_id: safeOrderId, path, filename: file.name, mime_type: file.type, byte_size: file.size, created_by: user.id }).select("id, filename").single();
