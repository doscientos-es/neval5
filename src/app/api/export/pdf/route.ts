...
async function report(type: string, supabase: NonNullable<Awaited<ReturnType<typeof createServerSupabaseClient>>>, filters: Filters) {
  ...
  if (type === "quotes") { let query = supabase.from("quotes").select("number, customer_name_snapshot, status, total, created_at").order("created_at", { ascending: false }); if (filters.from) query = query.gte("created_at", filters.from); if (filters.to) query = query.lte("created_at", `${filters.to}T23:59:59.999Z`); if (filters.status) query = query.eq("status", filters.status); if (filters.customerId) query = query.eq("customer_id", filters.customerId); const { data, error } = await query; if (error) throw error; return { title: "Presupuestos", headers: ["Número", "Cliente", "Estado", "Total EUR", "Fecha"], rows: data.map((row) => [row.number, row.customer_name_snapshot, row.status, row.total, row.created_at]) }; }
