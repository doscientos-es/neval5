...
export type ConfigurationData = {
  organization: { id: string; name: string; taxId: string; address: string; city: string; province: string; email: string; phone: string; timezone: string; currency: string };
...
export async function getConfigurationData(): Promise<ConfigurationData | null> {
  ...
  const [{ data: organization, error: organizationError }, { data: memberships, error: membersError }] = await Promise.all([
    supabase.from("organizations").select("id, name, tax_id, address, city, province, email, phone, timezone, currency").eq("id", mine.organization_id).single(),
  ...
  return {
    organization: { id: organization.id, name: organization.name, taxId: organization.tax_id || "", address: organization.address || "", city: organization.city || "", province: organization.province || "", email: organization.email || "", phone: organization.phone || "", timezone: organization.timezone, currency: organization.currency },
